...
def add_sentence_tags(premise, hyp, word_dim, tag_dim):
    '''
    ...
    BOS = np.tile(np.reshape(BOS, (1, 1, word_dim+tag_dim)),(premise.shape[0],1,1))    # Vector for beginning of sentence
    SEP = np.tile(np.reshape(SEP, (1, 1, word_dim+tag_dim)),(hyp.shape[0],1,1))        # Vector for separator between premise and hypo
    EOS = np.tile(np.reshape(EOS, (1, 1, word_dim+tag_dim)),(hyp.shape[0],1,1))        # Vector for end of sentence
